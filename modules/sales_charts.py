...
def render_sales_chart(
    ...
    if inv_col not in df.columns:
        inv_col = "available" if "available" in df.columns else "inventory_supply_at_fba"
    ...
    if stockout_raw.max() > 1:
        stockout_frac = stockout_raw # Already percentage? No, usually >1 means 20 for 20%.
    ...
    ad_spend = [round(x, 2) if pd.notna(x) else None for x in ads_df["ad_spend"].tolist()]
    ad_sales = [round(x, 2) if pd.notna(x) else None for x in ads_df["total_sales"].tolist()]
    ad_units = [round(x, 0) if pd.notna(x) else None for x in ads_df["total_units"].tolist()]
    ad_clicks = [round(x, 0) if pd.notna(x) else None for x in ads_df["clicks"].tolist()]
    ad_impressions = [round(x, 0) if pd.notna(x) else None for x in ads_df["impressions"].tolist()]
    acos = []
    ...
    if show_change_notes and "change_notes" in df.columns:
        for i, (date, note, unit_val) in enumerate(zip(dates, df["change_notes"], units)):
            ...
            if not show_lds and (note_str.startswith("LD") or note_str.startswith("BD")):
                continue
    ...
    option = {
        ...
        "legend": {
            "data": ["Units", "Sessions", "Inventory", "Stockout", "Ad Spend", "ACoS", "Ad Units", "Impressions", "Clicks"],
            "top": "0%",
            "textStyle": {"color": "#ccc"}
        },
        "toolbox": {
        ...
        "yAxis": [
            ...
            # --- Middle Grid Axis (Index 3) ---
            {
                ...
                "splitLine": {"show": False},
                "axisLabel": {"formatter": "{value} %"} # Show as percentage (positive)
            },
        ...
        "series": [
            ...
            # --- Middle Grid Series (Stockout) ---
            {
                ...
                "symbol": "none",
                "emphasis": {"focus": "series"},
                "itemStyle": {"color": "rgba(255, 0, 0, 0.25)"} # Force color for legend
            },
            # --- Bottom Grid Series ---
            {
                "name": "Ad Spend",

...
if login_st():
    ...
    with col1:
        with st.expander('OTP codes', icon=':material/qr_code_2:'):
            ...
            try:
                ...
                if sorted_keys:
                    ...
                    st.text_area(label='OTPs',value=output, height=200, key=str(time.time()))
        ...
        with st.expander('Meeting summarizer',expanded = False, icon=':material/summarize:'):
            st.warning("Deprecated, use Gemini or any other available chatbot instead", icon=":material/adb:")
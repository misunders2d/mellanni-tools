...
df_columns_config = {
    "Search Query": st.column_config.TextColumn(pinned=True),
...
def is_similar(query, target, threshold):
    ...
    if all(
        [
            any([fuzz.ratio(search_term, word) >= 70 for word in words])
...
def combine_files(
    ...
    summary = pd.merge(
        common_df, asin_df, how="outer", on=column, validate="1:1"
    ).fillna(0)
...
def filter_df(search_term, threshold=70):
    ...
    if search_term:
        bq = bq[
            bq["Search Query"].apply(lambda x: is_similar(x, search_term, threshold))
        ]
        kw = kw[
            kw["search_term"].apply(lambda x: is_similar(x, search_term, threshold))
        ]
    ...
    **kwargs,
    ...
    for i, right in enumerate(right_lines):
        fig.add_trace(
            go.Scatter(
                ...
                y=df[right],
                name=right,
                line=dict(color=colors_right[i]),
            ),
            secondary_y=True,
        )
    ...
    if len(right_lines) > 0:
        if right_axis == "percentage":
            fig.update_yaxes(
                title_text=right_lines[0], secondary_y=True, tickformat=".1%"
            )
        else:
...
if selected_dates:
    if button_area.button(
        ...
        key="pull_sqp_data",
        on_click=read_bq,
        args=(selected_dates[0], selected_dates[1]),
        kwargs=({"name": "bq_data"}),
        disabled=False,
    ):
    ...
    kw_filter_area.text_input(
        ...
        placeholder=(
            "Enter a keyword to filter the SQP data"
            if "bq_data" in st.session_state
            else "Please pull SQP data first"
        ),
    ...
    if "bq_data" in st.session_state:
        bq = (
            st.session_state["filtered_bq"].copy()
            if "filtered_bq" in st.session_state
            else st.session_state["bq_data"].copy()
        )
        ...
        fig1 = create_figure(
            ...
            left1="Search Query Volume",
            left2="Clicks: Brand Count",
            right1="KW ctr",
            right2="ASIN ctr",
        )
        fig2 = create_figure(
            bq_dates,
            title="Add-to-cart performance over time",
            left1="Cart Adds: Total Count",
            left2="Cart Adds: Brand Count",
            right1="KW ATC %",
            right2="ASINs ATC %",
        )
        fig3 = create_figure(
            bq_dates,
            title="Purchases",
            left1="Purchases: Total Count",
            left2="Purchases: Brand Count",
            right1="KW conversion",
            right2="ASINs conversion",
        )
        fig4 = create_figure(
            bq_dates,
            title="Pricing",
            left1="Clicks: Price (Median)",
            left2="Cart Adds: Price (Median)",
            left3="Purchases: Price (Median)",
            ...
            with st.spinner("Preparing data for download...", show_time=True):
                st.download_button(
                    ...
                    data=create_bytes_df(
                        [bq_dates, bq_search], ["Reporting Date", "Search Query"]
                    ),
                    file_name="SQP_data.xlsx",
                    mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                    key="download_sqp_data",
                    on_click=lambda: st.session_state.update(
                        {"export_sqp_data": False}
                    ),
                )
...
kw_columns_config = {
    "Reporting Date": st.column_config.DateColumn(
        format="YYYY-MM-DD", help="Date of the keyword data"
    ),
    "Position": st.column_config.NumberColumn(
        help="Average weighted position of the keyword", format="%.1f"
    ),
    "purchases_total_count": st.column_config.NumberColumn(
        help="Total number of purchases for the keyword", format="localized"
    ),
    "purchases_brand_count": st.column_config.NumberColumn(
        help="Total number of brand purchases for the keyword", format="localized"
    ),
    "search_term": st.column_config.TextColumn(
        help="Search term for the keyword", pinned=True
    ),
}
...
def pull_keywords_from_bq(list_of_dates) -> pd.DataFrame:
    ...
    with gc.gcloud_connect() as client:
        ...
        # print(len(result))
        result = (
            result.groupby(["date", "ASIN", "search_term"]).agg("min").reset_index()
        )
        # print(len(result))
        return result
...
if selected_dates and "bq_data" in st.session_state:
    ...
    kw_records = pull_keywords_from_bq(
        dates_list[
            dates_list.index(selected_dates[0]) : dates_list.index(selected_dates[1])
            + 1
        ]
    )
    sqp_data = st.session_state["bq_data"][
        [
            "Search Query",
            "Purchases: Total Count",
            "Purchases: Brand Count",
    ...
    kw_records = pd.merge(
        kw_records, sqp_data, how="left", on=["search_term", "Reporting Date"]
    ).fillna(0)
    ...
    kw_baseline = (
        kw_baseline.groupby(["Reporting Date", "search_term"])
        .agg(
            {
                "purchases_total_count": "min",
                "purchases_brand_count": "min",
                "Position": "min",
            }
        )
        .reset_index()
    ...
    del kw_daily["prod"]
    kw_daily = kw_daily.sort_values(
        ["Reporting Date", "purchases_total_count"], ascending=False
    )

    kw_filtered = (
        kw_baseline.groupby(["Reporting Date"])
        .agg(
            {
                "prod": "sum",
                "purchases_total_count": "sum",
                "purchases_brand_count": "sum",
            }
        )
        .reset_index()
    )

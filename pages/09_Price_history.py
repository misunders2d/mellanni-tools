...
DAYS_TO_PULL = 60
START_DATE = (
    (pd.to_datetime("today") - pd.Timedelta(days=DAYS_TO_PULL))
    .date()
    .strftime("%Y-%m-%d")
)
...
notes_area.write(
    """*For some products you may see a "null" brand popping up - that is Amazon Basics not giving away it's data (or Mellanni hasn't launched yet)"""
)
...
def get_asins(queue, mode="mapping"):
    ...
    creds = ServiceAccountCredentials.from_json_keyfile_dict(
        st.secrets["gsheets-access"], scope
    )
    ...
    book = client.open_by_url(
        "https://docs.google.com/spreadsheets/d/12AD3N0eUWXt2YjY64OEJpj8IWRRxZA_Qq5TSZJ0gINQ"
    )
    ...
    if mode == "asins":
        ...
        for product in products:
            product_asins = (
                data[data["Product"] == product][["ASIN"] + asin_cols]
                .values[0]
                .tolist()
            )
...
if "data" in st.session_state:
    if button_area.button("Prepare export"):
        ...
        button_area.download_button(
            "Export full data", output.getvalue(), file_name="Price history.xlsx"
        )
        ...
        c = (
            alt.Chart(plot_file, title=product)
            .mark_line()
            .encode(
                x=alt.X("datetime:T"),
                y=alt.Y("final_price:Q"),
                color=alt.Color("brandasin"),
            )
        )
        ...
        for index, row in link_file.iterrows():
            columns[index].markdown(
                f"[{' : '.join([str(row['brand']),row['asin']])}]({row['link']})"
            )

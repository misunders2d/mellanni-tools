...
require_login()
if not st.user.email in markets_access:
    st.stop()
...
change_types = [
    ...
    "Video added/Updated",
    "PPC budget increased",
    "PPC budget decreased",
    "Removed from parent",
    "Added to parent",
    "Rebate started",
    "Rebate finished",
...
allowed_markets = [
    x
    for x in markets_match["dictionaries"].keys()
    if x in markets_access.get(st.user.email, [])
]


@st.cache_resource
def pull_dictionary(marketplace: str = "US") -> pd.DataFrame:
...
def pull_changes(
    marketplace: str = "US",
    start: datetime.date = (
        pd.to_datetime("today") - pd.Timedelta(days=NUM_DAYS)
    ).date(),
...
def generate_skus(
    collections: list, sizes: list, colors: list, dictionary: pd.DataFrame
) -> None:
    skus = (
        dictionary[dictionary["collection"].isin(collections)]["sku"].unique().tolist()
    )
    if len(sizes) > 0:
        skus = (
            dictionary[
...
def generate_collections_from_skus(skus: list, dictionary: pd.DataFrame) -> list:
    collections = (
        dictionary[dictionary["sku"].isin(skus)]["collection"].unique().tolist()
    )
    return collections
    ...
    changes_df = pd.DataFrame(
        list(
            zip(
                ...
                [notes for x in range(len(skus))],
            )
        ),
        columns=["date", "sku", "change_type", "notes"],
    )
...
marketplace_col, date_col, change_type_col, button_col = markets_row.columns(
    [3, 1, 2, 1]
)
marketplace = marketplace_col.radio(
    "Select marketplace", allowed_markets, horizontal=True
)

if marketplace == "Shopify":
    change_types.extend(
        [
            "MSRP set as Sale Price + discount coupon to standard",
            "Prices back to Standard from MSRP, coupon off",
        ]
    )
elif marketplace == "WM":
    change_types.extend(
        ["Spec file upload", "Unpublished - OOS", "Published - Back in Stock"]
    )
...
change_type = change_type_col.selectbox(
    "Select a change", options=change_types, index=change_types.index(change_types[-1])
)
notes = change_type_col.text_input("Add notes, if necessary")
change_date = date_col.date_input("Date of the change", value="today")
add_button = button_col.button(
    "Add changes", type="primary", disabled=not button_access
)  # , on_click=hide_df)
...
st.session_state.changelog = pd.merge(
    st.session_state.changes, st.session_state.dictionary, how="left", on="sku"
)
...
collections_filtered = sorted(
    st.session_state.dictionary["collection"].sort_values().unique().tolist()
)
st.session_state.collections = collection_list.multiselect(
    "Collection", collections_filtered
)

sizes_filtered = sorted(
...
st.session_state.skus = sku_list.text_area(
    f"SKUs ({len(skus_filtered)} selected)", value=", ".join(skus_filtered)
)
st.session_state.skus = [
    x.strip() for x in re.split("\n|,|;|\t", st.session_state.skus)
]
collections_from_skus = generate_collections_from_skus(
    st.session_state.skus, st.session_state.dictionary
)

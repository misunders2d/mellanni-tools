...
if login_st():
    ...
    product_area, color_area, size_area = st.columns([1, 1, 1])
    ...
    if st.session_state.selected_product:
        ...
        st.session_state.selected_color = color_area.multiselect(
    ...
    if 'selected_color' in st.session_state and st.session_state.selected_color and st.session_state.selected_product:
        sizes = sorted(
            dictionary[
                (dictionary['collection']==st.session_state.selected_product)
                &
                (dictionary['color'].isin(st.session_state.selected_color))
                ]['size'].unique().tolist()
        )
    ...
    if st.session_state.selected_product and st.session_state.selected_color and st.session_state.selected_sizes:
        # st.write(f"You selected: {st.session_state.selected_product} in {st.session_state.selected_color} color, sizes: {', '.join(st.session_state.selected_sizes)}")
        ...
        if st.button('Submit'):
            ...
            colors = [sanitize_products(x) for x in st.session_state.selected_color]

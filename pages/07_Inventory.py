...
def generate_pdf(fnsku_dict, mode="combined"):  # fnskus, titles, qty):
    ...
    x_coords = [
        round(x, 3)
        for x in np.arange(xmargin, pdf_w - xmargin, ((pdf_w - xmargin) - xmargin) / 3)
        + 0.15
    ]
    y_coords = [
        round(y, 3)
        for y in np.arange(ymargin, pdf_h - ymargin, ((pdf_h - ymargin) - ymargin) / 10)
        + 0.2
    ]
    ...
    for record in fnsku_dict:
        ...
        with open(f"barcodes/{record['fnsku']}.png", "wb") as f:
            Code128(str(record["fnsku"]), writer=ImageWriter()).write(
                f, options=options_fnsku
            )
        ...
        for num_record in range(number_barcodes):
            for p, s in enumerate(title_str):
                pdf.text(x_coords[ix] + 0.1 + (p / 3), y_coords[iy] + 0.5 + (p / 10), s)
            pdf.text(x_coords[ix] + 0.1, y_coords[iy] + 0.6, "New")
            pdf.image(
                f"barcodes/{record['fnsku']}.png",
                x=x_coords[ix],
                y=y_coords[iy],
                w=width,
                h=height,
...
def generate_itf(sku_list, dictionary, layout, qty, leading="00"):
    ...
    if layout == "Letter":
        ...
        x_coords = [
            round(x, 3)
            for x in np.arange(
                xmargin, pdf_w - xmargin, ((pdf_w - xmargin) - xmargin) / 3
            )
            + 0.15
        ]
        y_coords = [
            round(y, 3)
            for y in np.arange(
                ymargin, pdf_h - ymargin, ((pdf_h - ymargin) - ymargin) / 10
            )
            + 0.2
        ]
        pdf = FPDF("P", "in", "Letter")
    ...
    for i, sku in enumerate(sku_list):
        ...
        with open(f'barcodes/{sku.replace("-","_")}.png', "wb") as f:
            Code39(str(upc), writer=ImageWriter(), add_checksum=False).write(
                f, options=options_itf
            )
        if layout == "Letter":
            for p, s in enumerate(sku_str):
                pdf.text(x_coords[ix] + 0.1, y_coords[iy] + (p / 7), s)
            pdf.image(
                f'barcodes/{sku.replace("-","_")}.png',
                x=x_coords[ix],
                y=y_coords[iy] + 0.2,
                w=width,
                h=height,
...
with col3:
    ...
    if col3.checkbox("Dictionary"):
        ...
        col3.download_button(
            "Download dictionary",
            dictionary,
            file_name=f"Dictionary_{st.session_state.market}.xlsx",
        )
...
with col1:
    if barcode_type in ("ITF14", "FNSKU"):
        skus = st.text_area(
            "Input SKUs",
            height=300,
            help="Input a list of SKUs you want to generate barcodes for",
        ).split("\n")
        sku_list = [x for x in skus if x != ""]
    elif barcode_type == "SKU upload with qty":
        qty_file_obj = st.file_uploader(
            'Upload file with SKUs and quantities (make sure it has columns "sku" and "qty")'
        )
        if qty_file_obj:
    ...
    if col1.button("Create barcodes", icon=":material/barcode:") and len(sku_list) > 0:
        with st.spinner("Please wait"):
            ...
            if barcode_type != "SKU upload with qty":
                ...
                st.session_state.file = pd.merge(
                    st.session_state.file, qty_file, how="outer", on="sku"
                )
            ...
            if barcode_type in ("FNSKU", "SKU upload with qty"):
                ...
                qty = qty_type
                st.session_state.pdf = generate_itf(
                    sku_list, dictionary, layout, qty, leading=leading
                )
    ...
    if "pdf" in st.session_state and st.session_state["pdf"] is not None:
        ...
        col2.download_button(
            label="Download PDF", data=PDFbyte, file_name=st.session_state.file_name
        )
        ...
        col2.download_button(
            label="Download SKU list", data=output.getvalue(), file_name="SKU list.xlsx"
        )
...
if st.user.email in (
    ...
    @st.cache_data
    def get_markets():
        markets_query = client.query(
            "SELECT DISTINCT marketplace FROM `reports.fba_inventory_planning`"
        ).result()
    ...
    markets = get_markets()
    market_radio = inv_report_area.radio(
        "Select marketplace", markets, index=markets.index("US"), horizontal=True
    )
    ...
    inv_report_area.button(
        "Download inventory",
        key="inv_button",
        icon=":material/inventory_2:",
        on_click=lambda: download_inv_report(inv_date, market_radio),
    )
else:
    inv_report_area.write(
        f"{st.user.email} is not allowed to access this section.\nIf you believe you need access to inventory reports, please contact Sergey"
    )

title_check_area.write("Check titles for variability")
parent = title_check_area.text_input(
    "Input baseline (or parent) title", value="", max_chars=200
)
...
if title_check_area.button("Check"):
    ...
    title_df = pd.DataFrame(
        data=diffs.items(), columns=["Title", "Similarity"]

...
choice = st.radio(
    "Switch between Returns comments and Negative Reviews classifier",
    ["Returns", "Reviews"],
    horizontal=True,
    disabled=True,
)
chart_area, filters_area = st.columns([0.75, 0.25])
country = filters_area.radio(
    "Country",
    options=COUNTRIES,
...
if collection or collection == []:
    ...
    filtered_dictionary = filter_dictionary(dictionary, **params)
size = filters_area.multiselect(
    "Size(s):",
    sorted(dictionary["size"].unique()),
...
if size or size == []:
    params["size"] = size
    filtered_dictionary = filter_dictionary(dictionary, **params)
color = filters_area.multiselect(
    "Color(s):",
    sorted(dictionary["color"].unique()),
...
if choice == "Returns":
    ...
    if st.button("Analyze returns"):
        with st.spinner(
            f"Please wait, pulling data from {country} for {len(asins)} variations...",
            _cache=True,
        ):
            with gc.gcloud_connect() as client:
                st.session_state.returns = client.query(query).to_dataframe()
        if (
            "returns" in st.session_state
            and len(
                st.session_state.returns[
                    st.session_state.returns["customer_comments"] != "nan"
                ]
            )
            > 0
        ):
            st.session_state.returns["customer_comments"] = st.session_state.returns[
                "customer_comments"
            ].astype(str)
            with st.spinner(
                f"Please wait, analyzing {len(st.session_state.returns)} complaints"
            ):
                ...
                reasons = em.assign_top_labels(
                    label_relevances, labels, "Return reason"
                )
                reasons["top reasons"] = reasons["top reasons"].apply(
                ...
                summary = (
                    reasons.groupby("Label")["quantity"]
                    .agg("sum")
                    .reset_index()
                    .sort_values("quantity", ascending=False)
                )
                ...
                reasons = pd.merge(
                    reasons,
                    dictionary.drop_duplicates("asin")[
                        ["asin", "collection", "size", "color"]
                    ],
                    how="left",
                    on="asin",
                )
    ...
    if st.button("Analyze reviews"):
        ...
        if "reviews" in st.session_state and len(st.session_state.reviews) > 0:
            with st.spinner(
                f"Please wait, analyzing {len(st.session_state.reviews)} reviews"
            ):
                review_temp = st.session_state.reviews.copy()
                review_temp[["title", "body"]] = review_temp[["title", "body"]].astype(
                    str
                )
                review_temp["customer_comments"] = (
                    review_temp["title"] + "\n" + review_temp["body"]
                )
                ...
                reasons["top reasons"] = (
                    reasons["top reasons"].astype(str).replace("[]", "[Unidentified]")
                )
                summary = (
                    reasons.groupby("top reasons")["rating"]
                    .agg(lambda x: len(x))
                    .reset_index()

...
def read_promos(
    ...
    negative_str = (
        f"""WHERE NOT REGEXP_CONTAINS({column},"{'|'.join(negative_filter)}")"""
    )
...
@st.cache_data(show_spinner=False)
def read_all_orders(
    order_list, report="reports", table="all_orders", skus=False
) -> pd.DataFrame:
    """
    ...
    if skus:
        ...
        client.close()
        df = df.pivot_table(
            values=["quantity", "sales", "discount"], index="asin", aggfunc="sum"
        ).reset_index()
    ...
    if skus:
        df.rename(
            columns={"sales": "Sales, $", "discount": "Discount, $"}, inplace=True
        )
        return df
    orders["purchase_date"] = (
        orders["purchase_date"].dt.tz_convert(pacific).dt.tz_localize(None).dt.date
    )
    orders["item_price"] = orders["item_price"].astype("float")
    orders["item_promotion_discount"] = orders["item_promotion_discount"].astype(
        "float"
    )
...
@st.cache_data(show_spinner=False)
def read_attribution(report="reports", table="attribution", start=None, end=None):
    ...
    data = data.rename(
        columns={
            "adGroupId": "ad_group_name",
            "totalAttributedSales14d": "Sales, $",
            "totalUnitsSold14d": "quantity",
        }
    )
    numerics = ["int", "float"]
    ...
    data_pivot = data.pivot_table(
        values=num_cols, index="ad_group_name", aggfunc="sum"
    ).reset_index()
...
def process_data(code_list=None, start=None, end=None, coupons=False, skus=False):
    ...
    with st.spinner(spinner_str):
        ...
        promos["promo_code"] = (
            promos["description"].str.extract(code_pattern).fillna(" ")
        )
        ...
        if len(order_list) == 0:
            ...
            with st.spinner(f"Pulling {len(order_list)} orders"):
                ...
                if skus:
                    orders_pivot["Net proceeds"] = (
                        orders_pivot["Sales, $"] - orders_pivot["Discount, $"]
                    )
                    orders_pivot["Discount, %"] = round(
                        orders_pivot["Discount, $"] / orders_pivot["Sales, $"] * 100, 1
                    )
                    return orders_pivot, orders_pivot

                total = pd.merge(
                    orders_pivot, promos_pivot, left_index=True, right_index=True
                ).reset_index()
                full = total.copy()
                ...
                total[["item_price", "item_promotion_discount"]] = round(
                    total[["item_price", "item_promotion_discount"]], 2
                )

                total.rename(
                    columns={
                        "item_price": "Sales, $",
                        "item_promotion_discount": "Discount, $",
                    },
                    inplace=True,
                )
                total["Net proceeds"] = total["Sales, $"] - total["Discount, $"]
                total["Discount, %"] = round(
                    total["Discount, $"] / total["Sales, $"] * 100, 1
                )
...
if col2.button("Get promo\ncode stats"):
    st.session_state.file_name, st.session_state.sheet_name = (
        "Promo_report.xlsx",
        "Promos",
    )
    ...
    with st.spinner("Please wait, pulling information..."):
        if codes == []:
            st.session_state.processed_data, st.session_state.full = process_data(
                code_list=None, start=d_from, end=d_to
            )
        else:
            st.session_state.processed_data, st.session_state.full = process_data(
                code_list=codes, start=d_from, end=d_to
            )
...
if col3.button("Get SKU data"):
    ...
    st.session_state.processed_data, st.session_state.full = process_data(
        code_list=None, start=d_from, end=d_to, coupons=coupons, skus=True
    )

if "processed_data" in st.session_state and not isinstance(
    st.session_state.processed_data, str
):
    if isinstance(st.session_state.processed_data, list):
        display = st.session_state.processed_data[0]
        result = ff.prepare_for_export(
            [
        ...
        result = ff.prepare_for_export(
            [st.session_state["processed_data"], st.session_state.full],
            [st.session_state.sheet_name, "Full data"],
        )
    st.write(display)
    ...
    metric1.metric(
        "Total Sales and Units", "{:,}".format(units), label_visibility="hidden"
    )
    metric2.metric("Discount, $ and %", "${:,}".format(discount))
    metric2.metric(
        "Discount, $ and %", "{:.1%}".format(percentage), label_visibility="hidden"
    )
